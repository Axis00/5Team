/*
calResult(): max()를 통해 음악 취향 계산, 숫자로 표현
setResult(): 계산된 취향을 통해 지정된 결과 페이지로 이동
goResult(): 사용자가 모든 질문을 응답하면 이동, 애니메이션 효과를 통해 qna에서 result로 페이지로 이동, setResult()로 이동
addAnswer(): 질문에 맞는 답변 박스 생성, 다음 질문으로 넘어가기
goNext(): 다음 질문으로 이동, 마지막 질문이면 goResult()로 이동
*/
